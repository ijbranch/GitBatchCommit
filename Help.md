...
| **Caveats** | Only works for Delphi projects. Within a project file the value is resolved the way MSBuild layers it - `Base`, `Base_<Platform>`, `Cfg_N`, `Cfg_N_<Platform>`, each overriding the last - for the project's default Config and Platform, so a stale or higher number on a group that is not built is correctly ignored. Where several separate `.dproj` FILES are found, the highest of the RESOLVED versions wins. The old highest-anywhere scan is a last resort for the whole repository, used only when NO project file in it can be resolved - it never competes with a resolved answer, so an unresolvable file carrying a larger number cannot override one |
